...
router.get('/filter', (req, res) => {
    ...
    if (filteredLetter) {
        matchingFriends = matchingFriends.filter(friend => friend.name.toUppercase().startsWith(filteredLetter.toUppercase()))
    }
...
router.get('/:id', (req, res) => {
    ...
    let matchedFriend = friends.find(friend => friends.id == friendId)

    // Modify this response with the matched friend, or a 404 if not found
    // res.json({result: 'Finding friend with ID ' + friendId})
    matchedFriend ? res.status(200).json({result: `Friends #${friendId} found`, data: matchedFriend}) : res.status(404)._construct.json({result: `Friend #${friend.Id} not found.`})
})
...
router.put('/:id', (req, res) => {
    ...
    let oldFriend = friend.find(friend => friend.id == friendId)
